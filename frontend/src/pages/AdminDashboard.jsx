...
const AdminDashboard = () => {
  ...
  const fetchTours = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:5000/api/tours', { headers });
      setTours(response.data);
    } catch (error) {
      console.error('Error fetching tours:', error);
      alert(error.response?.data?.message || 'Error fetching tours. Please try again.');
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      const formattedDates = formData.startDates
        .split('\n')
        .filter(d => d.trim())
        .map(dateStr => {
      ...
      if (editingTour) {
        await axios.put(`http://localhost:5000/api/tours/${editingTour._id}`, tourData, { headers });
      } else {
        await axios.post('http://localhost:5000/api/tours', tourData, { headers });
      }
      ...
      alert(error.message || error.response?.data?.message || 'Error saving tour. Please try again.');
    }
  };
  ...
  const handleDelete = async (tourId) => {
    if (window.confirm('Are you sure you want to delete this tour?')) {
      try {
        ...
        await axios.delete(`http://localhost:5000/api/tours/${tourId}`, { headers });
        fetchTours();
      } catch (error) {
        console.error('Error deleting tour:', error);
        alert(error.response?.data?.message || 'Error deleting tour. Please try again.');
      }
    }
  };
